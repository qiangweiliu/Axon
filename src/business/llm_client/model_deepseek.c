/* model_deepseek.c — DeepSeek API model adapter
 *
 * DeepSeek uses OpenAI-compatible streaming with reasoning_content field.
 * Content is in "content" for final replies, "reasoning_content" during
 * chain-of-thought. Streaming responses use chunked transfer encoding.
 */
...
static int deepseek_build_body(char *buf, size_t buf_len,
...
static int deepseek_extract_int(const char *json, const char *field)
{
    /* Build search key: "field": */
    char key[64];
...
const llm_model_t llm_model_deepseek = {
    .name            = "deepseek",
    .build_body      = deepseek_build_body,
    .extract_content = deepseek_extract_content,
    .extract_int     = deepseek_extract_int,
};

/* Self-register via .llm_models section */
LLM_MODEL_REGISTER(llm_model_deepseek);
